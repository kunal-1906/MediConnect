...
const doctorDashboardController = async (req, res) => {
  try {
    ...
    // Dummy earning logic: ₹500 per appointment
    const earningsToday = todaysAppointments.length * 500;
    ...
    console.error(error);
    res.status(500).send({ success: false, message: 'Dashboard data error' });
  }
};
